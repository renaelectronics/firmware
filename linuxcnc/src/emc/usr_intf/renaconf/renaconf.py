...
datadir = os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "share", "linuxcnc","renaconf")
...
from renaconf import pages
from renaconf import build_INI
from renaconf import build_HAL
...
class Data:
    ...
    def load_preferences(self):
        ...
        filename = os.path.expanduser("~/.renaconf-preferences")
    ...
    # write renaconf's hidden preference file
    def save_preferences(self):
        filename = os.path.expanduser("~/.renaconf-preferences")
        print filename
        d2 = xml.dom.minidom.getDOMImplementation().createDocument(
                            None, "int-renaconf", None)
    ...
    def load(self, filename, app=None, force=False):
        ...
        for f, m in self.md5sums:
            m1 = md5sum(f)
            if m1 and m != m1:
                warnings.append(_("File %r was modified since it was written by renaconf") % f)
        if warnings:
            warnings.append("")
            warnings.append(_("Saving this configuration file will discard configuration changes made outside renaconf."))
            if app:
