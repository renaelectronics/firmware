...
*/
unsigned int QextSerialBase::baudRate(void) const
{
...
Error                           Explanation
---------------------------     -------------------------------------------------------------
E_NO_ERROR                      No Error has occurred
...
void QextSerialBase::startRecording(QString fileName, bool allowOverwrite)
{
    recordingFile.setFileName(fileName);
    if(allowOverwrite)
    {
        recordingFile.open(QIODevice::WriteOnly);
    }
    else
    {
        recordingFile.open(QIODevice::Append);
    }
}

void QextSerialBase::stopRecording(void)
{
    recordingFile.close();
}
